...
size_t formatedDate(char *dateBuf, size_t size) {
	...
	return strftime(dateBuf, size, "%a %d %b %Y %H:%M:%S %Z",
			gmtime(&timer));
}
...
void handleRequest(int sendfd, struct sockaddr_in clientAddr,
	...
	inet_ntop(AF_INET, &clientAddr.sin_addr, response.clientIP,
			INET_ADDRSTRLEN);
	...
	fileNameStart = 5;
	if (strncmp(buffer, "GET /", fileNameStart) != 0
			&& strncmp(buffer, "get /", fileNameStart) != 0) {
		return sendBadRequest(response);
	}
	...
	if ((fd = open(fileName, O_RDONLY)) != -1) {
		...
		if (rc == -1) {
			...
			sprintf(response.message, "200 OK %li/%li", rc,
					stat_buf.st_size);
			logger(response);
		}
		close(fd);
