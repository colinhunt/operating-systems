...
The web server is started with three command line arguments. The first argument is a TCP
port on which it will listen to service requests made by clients. The second argument is a
directory from where the server serves documents to the clients. The last argument is a log
file in which the server logs all transactions with all clients.
*/
...
/* We can change the handler by linking a different one that implements this header */
